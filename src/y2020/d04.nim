...
proc valid(passport: Table[string, string]): bool =
  return
    ...
    (len(passport["hgt"]) == 4 and parseInt(passport["hgt"][0..1]) in 59 ..
        76 and passport["hgt"][2..3] == "in" or
       len(passport["hgt"]) == 5 and parseInt(passport["hgt"][0..2]) in 150 ..
           193 and passport["hgt"][3..4] == "cm") and
    len(passport["hcl"]) == 7 and passport["hcl"][0] == '#' and allIt(
            passport["hcl"][1..6], it in {'0'..'9', 'a'..'f'}) and
    passport["ecl"] in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"] and
    len(passport["pid"]) == 9 and allIt(passport["pid"], it in {'0'..'9'})

proc kv(expr: string): (string, string) =
  let kv = split(expr, ":")
  (kv[0], kv[1])

proc parsePassport(s: string): Table[string, string] =
  return s.split.mapIt(kv it).toTable
